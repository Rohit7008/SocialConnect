...
export async function middleware(req: NextRequest) {
  ...
  const isPublicPostRoute = PUBLIC_POST_ROUTES.some((route) => 
    pathname.startsWith(route) && 
    req.method === 'GET' && 
    pathname === route // Exact match for /api/posts (not /api/posts/[id])
  );

  const isPublicPostPattern = PUBLIC_POST_PATTERNS.some((pattern) => 
    pattern.test(pathname) && req.method === 'GET'
  );
